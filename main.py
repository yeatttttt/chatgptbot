...
def logwriter(logs):
    ...
    with open(os.path.join('C://', 'Users', '2okun', 'Desktop', 'chatgptbot', 'log.txt'), 'r') as log_file:
        log_read = log_file.read()
    if log_read.find(logs) != -1:
        pass
    else:
        with open(os.path.join('C://', 'Users', '2okun', 'Desktop', 'chatgptbot', 'log.txt'), 'a+') as log_file:
            log_file.write('\n' + logs)
            print(logs)
...
def user_data_logwriter(username, message, chat_response, user_id):
    ...
    filename = 'C://Users/2okun/Desktop/chatgptbot/user_data/'+str(date)+'_'+str(username)+'.txt'
...
def chatgpt(firstname, username, user_id, message):
    if 'ADVERT' in message and user_id == 826166970:
        print(message)
        print('ADVERT' in message, user_id == 826166970)
        return("Sent!")
    else:
        try:
            chat_response = 0
